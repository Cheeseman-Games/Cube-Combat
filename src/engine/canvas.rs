use crate::engine::render::{DrawCommand, DrawList, Rgba};
use web_sys::CanvasRenderingContext2d;

/// 2d-canvas backend. Flushes a full `DrawList` once per frame, so the game
/// code never crosses the wasm->JS boundary mid-draw.
///
/// This is the fallback path; the WebGPU backend replaces it when available.
        ...
        for command in &list.commands {
            match command {
                DrawCommand::Clear(color) => {
                    let _ = self
                        .ctx
                        .set_fill_style_str(&Self::css_color(*color));
                    self.ctx.fill_rect(0.0, 0.0, self.width as f64, self.height as f64);
                }
                DrawCommand::Rect { x, y, w, h, color } => {
                    let _ = self.ctx.set_fill_style_str(&Self::css_color(*color));
                    self.ctx.fill_rect(
                        (*x).into(),
                        (*y).into(),
                        (*w).into(),
                ...
                DrawCommand::Text { text, x, y, size, color } => {
                    let _ = self.ctx.set_font(&format!("{}px monospace", *size as u32));
